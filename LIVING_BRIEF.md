...
Last updated: March 15, 2026 (Session 13: tooling session. RUS stock market pipeline fully repaired and backfilled. Files changed: `update_market_sheet.py`, `sync_sheet.py`, new `backfill_rus_index.py`. Open issue: `stock_market_ytd = 2.33` is confirmed in data.json for RUS but not displaying in the USD column on the live site. Needs UI session to diagnose — see pending work.)

Session 13 changes in detail:
(1) `update_market_sheet.py`: added `fetch_moex_index()` which hits the MOEX public REST API (`iss.moex.com/iss/engines/stock/markets/index/securities/IMOEX/candles.json?interval=31`) for monthly candles. For RUS only, equity is now sourced from MOEX instead of yfinance (IMOEX.ME unreliable post-sanctions). `_moex_jan1` internal key carries the Jan-1 base level, bypassing sheet-history lookup. `Stock_Market_YTD_USD` is now computed for RUS using the MOEX local return adjusted by RUB/USD from the sheet (decision reversed mid-session — USD shown for completeness despite reliability caveat).
(2) `sync_sheet.py`: `--market` flag was silently ignored (it was never implemented). Added `run_market_sync()` function that reads the MARKET-STATS sheet via service account, grabs the last row of each country tab, and diffs/writes 4 fields into data.json: `stock_market_index`, `stock_market_ytd`, `fx_rate`, `bond_yield_10y`. `--market` and `--apply` (macro) modes are now mutually exclusive branches in `main()`. Macro path untouched.
(3) `backfill_rus_index.py`: one-off script (keep for re-use). Fetched 6,554 MOEX daily candles from 2000-01-01, matched against 3,891 blank `Stock_Market_Index` cells in the RUS tab, wrote 3,691 values in 4 batch chunks. 200 blanks remain (genuine non-trading days). Script is idempotent — only writes to blank cells.
(4) Build confirmed clean. RUS `stock_market_ytd = 2.33` (MOEX +3.2% local, adjusted for RUB/USD) written to data.json and live on site.

Last session note (Session 12): UI session. All changes in macrosnaps-shell.html only. (1) Deleted computeFxYtd and computeUsdReturn — USD toggle now reads co.metrics.market['Stock Market YTD (USD)'] directly. (2) Stock Market YTD ranking redesigned: two columns (Local + USD) always visible side by side, both sortable, plus sortable Countries column. RUS USD shows — as expected. (3) Hover tooltip suppressed on touch devices via @media(hover:none). (4) Logo click resets to GDP Growth home state. (5) DISPLAY_NAMES map added: 'United Kingdom'->'UK', 'United States'->'USA' in all ranked and grid table renders. (6) Value columns thinned (width:58px) and centre-aligned. All dead CSS (wh-usd-toggle, wh-usd-btn, wh-usd-note) removed.
...
1. **RUS stock_market_ytd not displaying in USD column (UI session).** Pipeline is confirmed working — `sync_sheet.py --market --apply` writes `stock_market_ytd = 2.33` into data.json for RUS. But the value is not appearing in the Stock Market YTD USD column on the live rankings table. Root cause unknown — likely the shell reads `stock_market_ytd` via a different key path or the USD column has a RUS-specific guard condition left over from when we were suppressing it. Upload `LIVING_BRIEF.md` + `macrosnaps-shell.html`. Search for how `stock_market_ytd` is read in the rankings table and check for any RUS exclusion logic. `fetch_moex_index()` added to `update_market_sheet.py`. `run_market_sync()` added to `sync_sheet.py` (the `--market` flag was previously silently ignored). `backfill_rus_index.py` backfilled 3,691 daily closes into the RUS tab. RUS now has `stock_market_index` and `stock_market_ytd` (2.33% USD) in data.json. **Known issue**: the RUS tab now has 6,500+ rows; `sync_sheet.py --market` occasionally hits Sheets API 429 quota on the RUS tab. Workaround: wait 60s and re-run. Future fix: add retry-with-backoff to `run_market_sync()`.
