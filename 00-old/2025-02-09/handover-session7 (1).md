...
- Upgrade prompt appears only when they try to toggle to Moderate/Expert.

**The viral loop:** Free beginner content -> user shares -> recipient sees beautiful snapshot -> taps link -> lands on beginner view -> explores -> hits Moderate/Expert paywall -> converts.
...
- Metrics in sheet: GDP_Growth_2026, Inflation_2026, Budget_Deficit_2026, Current_Account_2026, Unemployment_2026
- Note: `forecast_loader.py` currently only reads 4 of these 5 (missing unemployment) - needs updating to include unemployment
...
**General (1):** inflation
**Macro batch 1 (10):** interest rate, property crisis, real GDP, recession, automatic stabilizers, breakeven inflation, deflation, deposit rate, disinflation, dovish

### Recategorization (Session 5/6)
...
### 3. Commodities Card (new workstream)
- Add a 10th globe dot: "CMDTY" or "Commodities" - opens a card with key global commodities
- Candidates: Brent crude, WTI, gold, copper, iron ore, natural gas, wheat, LNG
- Same UX as country cards: weather icon, 3-level stories, clickable metrics with popovers/charts/compare
- Compare view becomes powerful: "gold price" ranked against... itself over time, or cross-commodity
- Glossary terms to add: contango, backwardation, commodity supercycle, Dutch disease, resource curse, terms of trade, Brent-WTI spread, OPEC+, strategic petroleum reserve
- Daily stories: commodity narratives woven into the 3 global stories where relevant
- Country cards stay uniform (same 14 metrics for all 9 countries, no per-country commodity rows)
- Data sources: Yahoo Finance (GC=F, CL=F, BZ=F, HG=F, SI=F), possibly FRED for some series

### 4. Outstanding UX/Content Work
- **US spelling pass:** Convert all British spellings in glossary (25 enriched entries + 145 thin), stories, UI text, and HTML content
- **Mobile responsiveness:** Test on iPhone, design mobile UX (likely needs different navigation paradigm for globe)
...
11. **Uniform country cards:** All 9 countries have identical metric structure (same 14 metrics). No per-country additions. Commodities get their own card instead.
12. **US spelling throughout.** Organization, labor, favor, stabilizers, defense, etc. The target market is global English-language finance, anchored by the US market. All existing content (glossary entries, stories, UI text) needs a spelling pass to convert from British to American.
13. **Mobile: NOT YET ADDRESSED.** The prototype is desktop-only. Globe, cards, popovers, compare views, and weather grids were all built for desktop screens. A dedicated mobile design pass is needed - this is a design question, not just CSS tweaks. **Mobile is the launch priority (60-70% of users).** Desktop-first for prototyping is fine but no new features should be fundamentally incompatible with mobile.
14. **Sharing as growth engine.** 3 shareable units: country snapshot, comparison chart, glossary explainer. Deep links to specific content. Beginner level always free and viewable without login.
...
- Enriched 10 macro batch 1 entries (interest rate, property crisis, real GDP, recession, automatic stabilizers, breakeven inflation, deflation, deposit rate, disinflation, dovish)
