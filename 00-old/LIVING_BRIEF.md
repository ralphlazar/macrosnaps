...
Last updated: March 11, 2026 (Tooling session - update_headlines.py and headline_review.html built and tested; final architecture: 3 API calls total: Haiku batch 1 USA/CAN/GBR/JPN/DEU/FRA, Haiku batch 2 CHN/IND/ZAF/BRA/RUS, Sonnet+search for global stories; country calls use no web search; pending first clean run)
...
7. ~~**Add country-level and global stories to the daily bash ritual.**~~ Done March 11, 2026. `update_headlines.py` calls the Claude API with web search enabled, drafts 12 country story blocks (3 bullets x 3 levels) and global stories (3 items x 3 levels), and saves to `stories_draft_YYYY-MM-DD.json`. `headline_review.html` is a browser-based review and edit tool: load the draft, edit inline, approve per country, export `stories_approved_YYYY-MM-DD.json`. Apply with `python3 update_headlines.py --apply stories_approved_YYYY-MM-DD.json`. Sources from web search are embedded in the draft for verification. Architecture finalised March 11, 2026. 3 API calls total: two Haiku batch calls (6 countries each, no web search, macro metrics from data.json as context), one Sonnet+search call for global stories. Country calls cost pennies. Global call is one search-enabled Sonnet call. Pending first fully clean run.
