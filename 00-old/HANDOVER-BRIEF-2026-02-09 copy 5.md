...
10. Fixed chart click bug - clicking chart line on default 5Y view caused chart to disappear. Two fixes: stopPropagation on canvas element, removed 'click' from Chart.js events array (hover tooltips still work via mousemove)
...
## NEXT REFACTOR: STEPS 4 & 5

### Step 4: Rendering Functions That Take Data, Not Find Data

**Problem:** `showMTT` currently does its own data lookups - finds the country, looks up metricExpl, checks historicalData, gets metricSources, checks fxRegime, all inside one function. If any lookup fails, the whole popover can break. Same pattern in `showCard`, `showCompare`, etc.

**Solution:** Separate data assembly from rendering. For each major renderer, create a data-assembly function that collects everything into one object, then pass that object to the renderer.

**Pattern:**
```
// BEFORE (current)
function showMTT(n, v, co, sec) {
  const exEntry = (metricExpl[currentLevel]||metricExpl.moderate)[n];
  const code = getCountryCode(co);
  const histEntry = code && historicalData[code] && historicalData[code][n];
  // ... 20 more lookups scattered through rendering ...
}

// AFTER (target)
function assembleMTTData(metricName, value, countryName, section) {
  const code = getCountryCode(countryName);
  return {
    metricName, value, countryName, section, code,
    explanation: (metricExpl[currentLevel]||metricExpl.moderate)[metricName] || null,
    source: isFxMetric(metricName) ? fxSource : (metricSources[metricName] || null),
    fxRegimeInfo: (isFxMetric(metricName) && code && fxRegime[code]) ? fxRegime[code] : null,
    historical: (code && historicalData[code] && historicalData[code][metricName]) || null,
    metricList: getMetricList(countryName),
    currentIndex: getMetricList(countryName).findIndex(m => m.name === metricName)
  };
}

function showMTT(n, v, co, sec) {
  const data = assembleMTTData(n, v, co, sec);
  renderMTT(data);
}

function renderMTT(d) {
  // Pure rendering - only reads from d, never does lookups
}
```

**Functions to refactor (in priority order):**
1. `showMTT` -> `assembleMTTData` + `renderMTT` (most complex, 14 metrics x 9 countries)
2. `showCard` -> `assembleCardData` + `renderCard`
3. `showCompare` -> `assembleCompareData` + `renderCompare`
4. `showWeatherGDP` / `showWeatherCPI` -> `assembleWeatherGrid` + `renderWeatherGrid` (unify into one function with a metric parameter)
5. `showNewsTooltip` -> `assembleNewsData` + `renderNews`
6. `showBubble` (glossary) -> `assembleGlossaryData` + `renderBubble`

**Keep in mind:**
- Event handlers (close, navigate, level toggle, compare button) stay inside the render function - they need DOM references
- `renderMetricChart` is already clean (takes container, metricName, code, range) - no change needed
- The assemble functions are the testability layer - you can unit test them without a DOM
- Navigation callbacks (metric arrows, level toggle rerender) call the assemble+render cycle again

### Step 5: Defensive Rendering Everywhere

**Problem:** A missing data field (no historical data, no fxRegime, no stories) can cause a JS error that kills the entire popover or card.

**Solution:** Every section in every renderer checks for data existence before rendering. Missing data = skip the section, not crash.

**Pattern for renderMTT:**
```
function renderMTT(d) {
  let html = '';
  
  // Always present
  html += `<div class="tt-title">${d.metricName}</div>`;
  html += `<div class="tt-section"><div class="tt-label">${d.countryName}</div><div class="tt-value">${d.value}</div></div>`;
  
  // Chart - only if historical data exists
  if (d.historical) {
    html += buildChartHTML(d.historical);
  }
  
  // Explanation - only if metricExpl exists
  if (d.explanation) {
    const bluf = getBluf(d.explanation);
    const full = getFull(d.explanation);
    html += `<div class="tt-story">${bluf}`;
    if (full) html += `<div class="ge-expand">Read more ›</div><div class="ge-full">${full}</div>`;
    html += `</div>`;
  }
  
  // FX regime - only if relevant
  if (d.fxRegimeInfo) {
    html += buildFxRegimeHTML(d.fxRegimeInfo);
  }
  
  // Source - only if available
  if (d.source) {
    html += `<div class="tt-source">Source: ${d.source}</div>`;
  }
  
  // Compare button - always present
  html += buildCompareButton(d);
  
  // ... render to DOM, attach event handlers ...
}
```

**Defensive rules to apply across ALL renderers:**
1. `d.explanation || null` - never crash on missing metricExpl
2. `d.historical || null` - chart section simply hidden
3. `d.fxRegimeInfo || null` - FX regime section simply hidden
4. `d.source || null` - source line simply hidden
5. `d.stories || ['Data loading...']` - fallback story text (already exists in showCard)
6. `glossary[term] || null` - glossary term not found = render as plain text, not crash
7. `d.metricList.length > 0` - navigation arrows hidden if no metric list
8. `d.weatherGrid || null` - weather grid data missing = hide the grid

**Testing approach:** After refactoring, temporarily delete one country's historical data from the JSON block and verify the popover still renders (minus the chart). Do the same for fxRegime, metricExpl entries, etc. Each deletion should cause graceful degradation, never a blank screen or console error.
