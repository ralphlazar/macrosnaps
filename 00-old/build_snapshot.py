...
Fetches all 14 metrics for 9 countries from FRED + Yahoo Finance,
...
FALLBACK = {
    "corp_spread": {
        ...
        "DEU": 55, "FRA": 60, "ITA": 65, "CHN": 55, "IND": 75,
    },
    "sov_cds": {
        "USA": 35, "CAN": 40, "GBR": 25, "JPN": 22,
        "DEU": 15, "FRA": 30, "ITA": 55, "CHN": 60, "IND": 95,
    },
    "yield_curve": {
        "CAN": 15, "GBR": 10, "JPN": 53,
        "DEU": 30, "FRA": 38, "ITA": 62, "CHN": 46, "IND": 23,
    },
    # Known FRED data gaps
    "inflation":      {"JPN": 2.8},
    "gdp_growth":     {"CHN": 5.2},
    "unemployment":   {"CHN": 5.1, "IND": 4.7},
    "bond_yield_10y": {"CHN": 2.56, "IND": 7.18},
}
...
def build_country(code, cfg, prev_snapshot, skip_historical=False):
    ...
    return {
        ...
        "weatherGrid": prev.get("weatherGrid", {
...
def build_snapshot(output_path="snapshot.json", skip_historical=False):
    ...
    for code, cfg in COUNTRY_CONFIG.items():
        ...
        try:
            country_data, freshness = build_country(code, cfg, prev, skip_historical)
