#!/usr/bin/env python3
"""
rebuild_policy_rate_historical.py
==================================
Rebuilds _frozen_historical['Policy Rate'] for all 12 countries by
reading the full MACRO-MONTHLY Policy_Rate tab directly from Google Sheets.
...
Countries with no data (CHN, IND) are skipped — existing entries untouched.

Run from the macrosnaps repo directory:
    python3 ~/Downloads/rebuild_policy_rate_historical.py          # dry run
    python3 ~/Downloads/rebuild_policy_rate_historical.py --apply  # write to data.json
"""
...
# These countries have no policy rate source — skip them, leave existing entry alone.
KNOWN_BLANKS = {'CHN', 'IND'}
...
def read_tab(service):
    ...
    if len(rows) < 2:
        print('ERROR: Policy_Rate tab is empty.')
        sys.exit(1)
    ...
    for row in rows[1:]:
        ...
        for country in COUNTRIES:
            ...
            if cell in ('', '-', 'N/A', 'n/a'):
                continue
            try:
                data[country][d] = round(float(cell), 4)
            except ValueError:
                pass
...
def main():
    ...
    print(f'\nrebuild_policy_rate_historical.py  [{mode}]\n')

    if not SHEET_ID:
        print('ERROR: MACRO_MONTHLY_SHEET_ID not set in .env')
        sys.exit(1)

    print('Reading MACRO-MONTHLY Policy_Rate tab...')
    service      = get_service()
    ...
    print(f'  Updated _frozen_historical Policy Rate for {updated} countries.')
